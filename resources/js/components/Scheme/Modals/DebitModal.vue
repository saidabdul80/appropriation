<template>
  <div class="modal show" id="debit-modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
    aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel3"><b>{{ appropriation?.name }} / {{ appropriation?.department
              }}</b> Transactions</h5>
        ...
        <div class="modal-body">
          <InlineMessage severity="error" v-if="!globals.selected_scheme?.id" class="p-1 mb-3">
            <span style="font-size: 0.8em;" class="ms-1"> <b>Note:</b> Please Select a Programme to add a transaction
            </span>
          </InlineMessage>
          ...
          <div class="mb-3 input-group" v-if="globals.selected_scheme?.fund_category == 'year'">
            ...
            <select v-model="selected_fund_category" @input="monthYearTriggered($event)" class="form-control">
              <option value=""></option>
              <option v-for="month_year in fund_categories" :value="month_year"
                :selected="appropriation_log2.fund_category" :key="month_year">{{ month_year }}</option>
            </select>
          </div>
          <div class="mb-3">
          ...
          <div v-if="appropriation?.budget_location === 'subhead'">
            <div class="mb-3">
              ...
              <Dropdown v-model="selected_subhead" class="w-100" :options="appropriation.subheads"
                optionLabel="subhead" />
            </div>

            <div class="mb-3">
              <label for="forApp" class="form-label">Subhead Budget Activity .</label>
              <Dropdown v-model="selected_subhead_item" class="w-100" :options="selected_subhead.subhead_budget_items"
          ...
          <div v-if="!uploading">
            ...
            <div v-if="reloadDynamicData" class="mb-3 w-100 d-flex">
              ...
              <MultiSelect filter v-model="dynamicDataSelected" icon="pi pi-filter" display="chip"
                @update:model-value="dynamicDataSelectedFunc" :options="data_columns"
                placeholder="Other Fields (Optional) Filter" :maxSelectedLabels="2" class="w-100" /><!--
            ...
            <div v-for="key in Object.keys(appropriation_log2.data || {})" :key="key"
              ...
              <div
                ...
                <div class="mb-3 position-relative">
                  ...
                  <span @click="removeField(key)" class="pi fs1 close_item"
                    :class="appropriation_log.data[key].required === 0 ? 'pi-times' : 'pi-ban disabled'"></span>
                  <input v-if="appropriation_log.data[key].type === 'number'" step="any"
                    v-model="appropriation_log2.data[key].value" :type="appropriation_log.data[key].type"
                    class="form-control">
                  <input v-else v-model="appropriation_log2.data[key].value" :type="appropriation_log.data[key].type"
                    class="form-control">
                </div>
              </div>
              <div v-if="key === 'Amount'">
                <div class="mb-3 ">
                  <label for="ApplogAmount" class="form-label">Actual Amount (<span class="fs-9 text-dark mb-1">{{
                    $globals.currency(appropriation_log.data.Amount.value) }}</span>)</label>
                  <div class="input-group position-relative mt-1 mb-1" id="ApplogAmount">
                    <span class="input-group-text">&#8358;</span>
                    <span @click="removeField(key)" class="pi fs1 close_item"
                      :class="appropriation_log.data[key].required === 0 ? 'pi-times' : 'pi-ban'"></span>
                    <input v-model="appropriation_log2.data.Amount.value" id="percentage_dividend" type="number"
                      step="any" class="form-control" aria-label="Amount (to the nearest dollar)">
                    <span v-if="appropriation_log.id === ''" id="schemeBalanceOver"
                      class="input-group-text">{{ availableBalance
                        < 0 ? '0.00' : $globals.currency(availableBalance - calculatedValues.gross) }}</span>
                        <span v-else id="schemeBalanceOver" class="input-group-text">{{
                          $globals.currency((availableBalance +
                          appropriation_log.total_amount) - calculatedValues.gross) }}</span>
                        <span class="d-none">{{ leftAmountApp < 1 ? 0.00 : leftAmountApp }}</span>
                  </div>
                  <div id="schemeBalanceOver2" class="alert alert-danger py-0 px-1 text-center fs-9 d-none">Insufficient
                    Balance
                  </div>
                </div>
              </div>
              <div v-if="key === 'VAT_%'">
                <div class="mb-3 ">
                  ...
                  <div class="input-group mt-1 position-relative" id="ApplogVat">
                    ...
                    <span @click="removeField(key)" class="pi fs1 close_item"
                      :class="appropriation_log.data[key].required === 0 ? 'pi-times' : 'pi-ban'"></span>
                    <input v-model="appropriation_log2.data[key].value" :type="appropriation_log.data[key].type"
                      class="form-control">
              ...
              <div v-if="key === 'Stamp_Duty_%'">
                <div class="mb-3 position-relative">
                  ...
                  <p class="fs-9 text-danger mb-1">Sub Total 1 (Actual Amount + VAT_% + Withh. tax): {{
                    $globals.currency(calculatedValues.subTotalCalculate) }}</p>
                  ...
                  <p class="fs-9 text-danger mb-1">Sub Total 2 (Actual Amount + VAT_% + Withh. + Stamp D. tax): {{
                    $globals.currency(calculatedValues.subTotalCalculate + calculatedValues.stampDutyCalculate) }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="modal-footer">
          ...
          <button v-if="appropriation_log.id === ''" type="button" @click="transact"
  ...
  methods: {
    async handleFileUpload(event) {
      ...
      if (file) {
        ...
        formData.append('fund_category', this.selected_fund_category);
        formData.append('owner_id', this.appropriation.id);
        formData.append('owner_type', 'appropriation');
        formData.append('subhead_id', this.selected_subhead.id);
        formData.append('subhead_item_id', this.selected_subhead_item.id);        
        formData.append('file', file);
        let res = await postData('/upload_appropriation_transaction',formData, false, true)        
        event.target.value = '';
      }
    },
    downloadCSV() {
